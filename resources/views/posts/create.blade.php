...
@section("content")
    <main class="container mx-auto">
        <section class="pt-4">
            <h2 class="text-2xl">Crear Post</h2>
            <form class="bg-white shadow-md rounded px-8 pb-8 mb-4" method="POST" enctype="multipart/form-data" action="{{ route('posts.store') }}" autocomplete="on">
                @csrf
                <div class="shadow sm:rounded-md sm:overflow-hidden">
                    <div class="px-4 bg-white space-y-6 sm:p-6">
                        ...
                        <div class="grid grid-cols-3 gap-6">
                            <div class="col-span-3 sm:col-span-2">
                                ...
                                <div class="mt-1 flex rounded-md shadow-sm">
                                    <input type="text" name="title" id="title" class="focus:ring-red-500 focus:border-red-500 flex-1 block w-full rounded-none rounded-r-md sm:text-sm border-gray-300 p-1" placeholder="Introduce un título..." value="{{ old("title") }}">
                                </div>
                            </div>
                        </div>
                        @error('title')
                        ...
                        <div>
                            ...
                            <div class="mt-1">
                                <textarea id="content" name="content" rows="3" class="shadow-sm focus:ring-red-500 focus:border-red-500 mt-1 block w-full sm:text-sm border-gray-300 rounded-md p-1" placeholder="Introduce el contenido...">{{ old("content") }}</textarea>
                            </div>
                        </div>
                        @error('content')
                            <div class="text-center inline-block">
                        ...
                        <div>
                            ...
                            <div class="mt-1 flex rounded-md shadow-sm">
                                <input type="text" name="category" id="category" class="focus:ring-red-500 focus:border-red-500 flex-1 block w-full rounded-none rounded-r-md sm:text-sm border-gray-300 p-1" placeholder="Introduce una categoría..." value="{{ old("category") }}">
                            </div>
                        </div>
                        @error('category')
                            <div class="text-center inline-block">
